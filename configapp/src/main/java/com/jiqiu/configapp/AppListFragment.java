...
public class AppListFragment extends Fragment implements AppListAdapter.OnAppToggleListener {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (!configManager.isRootAvailable()) {
            Toast.makeText(getContext(), "需要 Root 权限才能配置注入", Toast.LENGTH_LONG).show();
        }
    ...
    private void setupRecyclerView() {
        ...
        adapter.setToggleEnabled(configManager.isRootAvailable());
    ...
    public void onAppToggle(AppInfo appInfo, boolean isEnabled) {
        if (!configManager.isRootAvailable()) {
            Toast.makeText(getContext(), "需要 Root 权限", Toast.LENGTH_LONG).show();
            return;
        }
        // 保存应用的启用状态到配置文件
